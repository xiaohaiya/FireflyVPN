...
export const DEFAULT_RUNTIME_CONFIG: RuntimeConfig = {
  ...
  security: {
    ...
    adminLoginRateLimitPerMinute: 3,
  },
};
...
export function normalizeRuntimeConfig(value: unknown): RuntimeConfig {
  ...
  return {
    ...
    security: {
      adminLoginRateLimitEnabled: security.adminLoginRateLimitEnabled !== false,
      adminLoginRateLimitPerMinute: Number.isSafeInteger(security.adminLoginRateLimitPerMinute)
        ? Math.min(Math.max(Number(security.adminLoginRateLimitPerMinute), 1), 60)
        : DEFAULT_RUNTIME_CONFIG.security.adminLoginRateLimitPerMinute,
    },
  };
}
