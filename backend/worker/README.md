...
- 管理登录使用可动态配置的 D1 原子计数限流；注册 IP/deviceId 与已认证设备使用 Cloudflare Rate Limiting Binding，本地测试保留 D1 回退；另有 64 KiB JSON 上限和严格输入校验。
- 完整首版 D1 schema migration。
- KV 运行时配置模型与安全的默认配置。
- Crypto V2 Base64URL、P-256 SPKI 导入、ECDH、HKDF-SHA256、AES-256-GCM 和固定 AAD。
- 纯托管或外源 + 托管两种订阅模式、KV 缓存、超时/大小/HTML/URL 安全检查。
...
外源响应限制为 2 MiB，带超时、HTML/明显错误页检查和 TTL 缓存。网络/超时以及 HTTP 408、429、5xx 会在 500 ms、1500 ms 退避后最多重试两次，三次均失败后才判定拉取失败。真实外源 URL 不会下发给客户端。外源正文与刷新时间写在同一个 KV 键中，边缘缓存仍按源配置的 TTL 刷新，KV 持久化最多每小时一次，以避免 5 分钟刷新周期耗尽免费层写入额度；强制刷新仍会立即持久化。纯托管订阅可直接创建和编辑，无需外源链接。每个外源都可用 `externalHealthEnabled` 单独控制健康监测与故障回退，新建外源默认启用；Cron 每小时主动检查，普通拉取失败时下发最近一次成功内容，并在管理列表记录正常、异常和回退状态。强制刷新始终访问源站，失败时返回错误，不用缓存掩盖检测结果。

托管正文只保存在 `subscription:managed:<id>`。列表、创建、修改响应和审计日志均不包含正文；只有受管理员 JWT 保护的单条详情接口会返回已配置的 `managedContent`，供控制台编辑时回填。运行配置与外源、托管正文使用短时实例缓存及 Cloudflare 数据中心本地 Cache API 吸收重复 KV 读取，源的 `updatedAt` 参与缓存键，后台修改后不会继续命中旧版本。`refresh` 只刷新外源缓存，返回合并后的内容摘要。
...
`scripts/update_subscription.py` 通过受保护的管理 API 修改单个订阅源，不直接操作 D1 或 KV，也不会改动名称、启用状态、排序、备注和缓存 TTL。脚本仅依赖 Python 3.10+ 标准库。
...
Cron Trigger 每小时清理过期 Challenge 与 D1 限流窗口，并检查已启用健康监测的外源。管理登录使用 D1，以支持动态开关和次数；设备相关生产请求使用 Cloudflare 原生限流。设备活跃时间每 15 分钟至多更新一次，流量上报不会重复更新该字段。高频临时表使用 `WITHOUT ROWID`，并以较低的写入成本保留原子防重放与幂等约束。始终跳过邮箱、注册资料和密码相关账号功能。禁止在 Worker 启动时创建或修改表。
...
演示集包含 4 个用户账号、4 台不同平台/状态设备、14 天日流量、6 个月月流量及一条外源加托管的演示订阅。示例地址使用 `example.com`，仅供展示，不可实际连接。脚本是幂等的，重复执行不会不断增加账号和设备。
