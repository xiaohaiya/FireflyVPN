...
const MANAGED_MEMORY_CACHE_SECONDS = 300;
...
const memoryCaches = new WeakMap<KeyValueStore, Map<string, ContentCacheEntry>>();
const refreshes = new WeakMap<KeyValueStore, Map<string, Promise<ExternalContentResult>>>();
...
function cacheMap(config: KeyValueStore): Map<string, ContentCacheEntry> {
...
function readRemembered(config: KeyValueStore, key: string): string | null {
...
function invalidateMemoryCache(config: KeyValueStore, sourceId: string): void {
...
async function readManagedContent(
  ...
  return remember(config, key, content ?? "", MANAGED_MEMORY_CACHE_SECONDS) || null;
}
...
async function readExternalContent(
  ...
  if (!forceRefresh) {
    ...
    if (
      ...
      return {
        content: remember(config, cacheKey, cached.value, remainingTtl),
        fallbackUsed: false,
  ...
  const refresh = (async (): Promise<ExternalContentResult> => {
    ...
    try {
      ...
      // A successful origin fetch remains usable for this process even if the
      // durable cache write fails.
      console.error(JSON.stringify({
...
export async function clearSourceContent(config: KeyValueStore, sourceId: string): Promise<void> {
  await Promise.all([
