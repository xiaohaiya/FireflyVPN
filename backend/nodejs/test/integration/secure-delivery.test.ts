...
const EXTERNAL_CONTENT = "vless://external@example.com:443#External";
const NEXT_EXTERNAL_CONTENT = "vless://next@example.com:443#Next";
...
describe("secure subscription delivery", () => {
  ...
  it("protects admin APIs, combines external and managed content, and blocks replay", async () => {
    ...
    const limitedIp = "198.51.100.200";
    for (let attempt = 0; attempt < 4; attempt += 1) {
      const wrongRoute = await createApp().request("/wrong-admin-route/api/session/login", {
        method: "POST",
        headers: {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const rejectedLogin = await createApp().request(`/${ADMIN_ROUTE}/api/session/login`, {
        method: "POST",
        headers: {
          Authorization: "Bearer wrong-password",
          "CF-Connecting-IP": limitedIp,
    ...
    const rateLimitedLogin = await createApp().request(`/${ADMIN_ROUTE}/api/session/login`, {
      method: "POST",
      headers: {
        Authorization: "Bearer wrong-password",
        "CF-Connecting-IP": limitedIp,
    ...
    expect(await rateLimitedLogin.json()).toMatchObject({ error: "rate_limited" });
    const storedLoginIdentity = await env.DB.prepare(`SELECT identity_hash AS identityHash
      FROM crypto_rate_limits WHERE scope = 'admin_login:ip' LIMIT 1`)
      .first<{ identityHash: string }>();
    expect(storedLoginIdentity?.identityHash).toBeTruthy();
    expect(storedLoginIdentity?.identityHash).not.toContain(limitedIp);
    expect(storedLoginIdentity?.identityHash).not.toContain(ADMIN_ROUTE);
    const independentIpLogin = await createApp().request(`/${ADMIN_ROUTE}/api/session/login`, {
      method: "POST",
      headers: {
        Authorization: "Bearer wrong-password",
    ...
    expect(independentIpLogin.status).toBe(401);
    ...
    const runtimeSettingsPayload = {
      ...
      security: { adminLoginRateLimitEnabled: false, adminLoginRateLimitPerMinute: 4 },
    ...
    expect(await settings.json()).toMatchObject({
      ok: true,
      data: { security: { adminLoginRateLimitEnabled: false, adminLoginRateLimitPerMinute: 4 } },
    });
    for (let attempt = 0; attempt < 5; attempt += 1) {
      const loginWithoutLimit = await createApp().request(`/${ADMIN_ROUTE}/api/session/login`, {
        method: "POST",
        headers: {
          Authorization: "Bearer wrong-password",
      ...
      expect(loginWithoutLimit.status).toBe(401);
    }
    runtimeSettingsPayload.security = {
      adminLoginRateLimitEnabled: true,
      adminLoginRateLimitPerMinute: 2,
    };
    const enabledLimit = await createApp().request(`/${ADMIN_ROUTE}/api/settings`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${adminCredential}`,
        "Content-Type": "application/json",
    ...
    expect(enabledLimit.status).toBe(200);
    ...
    const customRateLimitedLogin = await createApp().request(`/${ADMIN_ROUTE}/api/session/login`, {
      method: "POST",
      headers: {
        Authorization: "Bearer wrong-password",
        "CF-Connecting-IP": "198.51.100.203",
    ...
    expect(customRateLimitedLogin.status).toBe(429);
    ...
    vi.stubGlobal("fetch", vi.fn<typeof fetch>().mockImplementation(async (url) =>
      new Response(String(url).includes("/next") ? NEXT_EXTERNAL_CONTENT : EXTERNAL_CONTENT)));

    const missingExternal = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${adminCredential}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ id: "missing-external", name: "无外源", sourceType: "external", managedContent: MANAGED_CONTENT }),
    }, env);
    expect(missingExternal.status).toBe(400);

    const missingManaged = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions`, {
      method: "POST",
      headers: { Authorization: `Bearer ${adminCredential}`, "Content-Type": "application/json" },
      body: JSON.stringify({ id: "missing-managed", name: "无托管正文", sourceType: "managed" }),
    }, env);
    expect(missingManaged.status).toBe(400);
    ...
    await expect(decryptSubscription(envelope, privateKey))
      .resolves.toBe(`${MANAGED_CONTENT}\n${EXTERNAL_CONTENT}`);
    const healthySource = await env.DB.prepare(`
      SELECT external_health_status AS status,
             external_fallback_active AS fallbackActive,
             external_last_success_at AS lastSuccessAt
      FROM subscription_sources WHERE id = 'main'
    `).first<{ status: string; fallbackActive: number; lastSuccessAt: string | null }>();
    expect(healthySource).toMatchObject({ status: "healthy", fallbackActive: 0 });
    expect(healthySource?.lastSuccessAt).toBeTruthy();

    const changedExternal = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/main`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${adminCredential}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ sourceUrl: "https://example.com/next" }),
    }, env);
    expect(changedExternal.status).toBe(200);
    expect(await env.CONFIG.get("subscription:managed:main")).toBe(MANAGED_CONTENT);
    const changedChallenge = encodeBase64Url(crypto.getRandomValues(new Uint8Array(16)));
    const changedContent = await createApp().request("/api/v2/subscriptions/main/content", {
      headers: deviceHeaders({
        "X-Firefly-Crypto-Version": "2",
        "X-Firefly-Challenge": changedChallenge,
      }),
    }, env);
    expect(changedContent.status).toBe(200);
    await expect(decryptSubscription(await changedContent.json() as CryptoV2Envelope, privateKey))
      .resolves.toBe(`${MANAGED_CONTENT}\n${NEXT_EXTERNAL_CONTENT}`);

    const clearedManaged = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/main`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${adminCredential}`,
        "Content-Type": "application/json",
    ...
    expect(clearedManaged.status).toBe(200);
    expect(await env.CONFIG.get("subscription:managed:main")).toBeNull();
    const clearedChallenge = encodeBase64Url(crypto.getRandomValues(new Uint8Array(16)));
    const externalOnlyContent = await createApp().request("/api/v2/subscriptions/main/content", {
      headers: deviceHeaders({
        "X-Firefly-Crypto-Version": "2",
        "X-Firefly-Challenge": clearedChallenge,
      }),
    }, env);
    expect(externalOnlyContent.status).toBe(200);
    await expect(decryptSubscription(await externalOnlyContent.json() as CryptoV2Envelope, privateKey))
      .resolves.toBe(NEXT_EXTERNAL_CONTENT);
    ...
    const optionalManaged = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${adminCredential}`,
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        ...
        name: "仅外源",
        sourceUrl: "https://example.com/subscription",
        externalHealthEnabled: false,
      }),
    }, env);
    expect(optionalManaged.status).toBe(201);
    expect(await optionalManaged.json()).toMatchObject({
      data: { sourceType: "external", externalHealthEnabled: 0, externalHealthStatus: "disabled" },
    });
    expect(await env.CONFIG.get("subscription:managed:external-only")).toBeNull();
    const managedOnly = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${adminCredential}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ id: "managed-only", name: "纯托管", sourceType: "managed", managedContent: MANAGED_CONTENT }),
    }, env);
    expect(managedOnly.status).toBe(201);
    expect(await managedOnly.json()).toMatchObject({ data: { sourceType: "managed", sourceUrl: null } });
    const managedChallenge = encodeBase64Url(crypto.getRandomValues(new Uint8Array(16)));
    const managedResponse = await createApp().request("/api/v2/subscriptions/managed-only/content", {
      headers: deviceHeaders({
        "X-Firefly-Crypto-Version": "2",
        "X-Firefly-Challenge": managedChallenge,
      }),
    }, env);
    expect(managedResponse.status).toBe(200);
    await expect(decryptSubscription(await managedResponse.json() as CryptoV2Envelope, privateKey))
      .resolves.toBe(MANAGED_CONTENT);
    const clearRequired = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/managed-only`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${adminCredential}`, "Content-Type": "application/json" },
      body: JSON.stringify({ managedContent: "" }),
    }, env);
    expect(clearRequired.status).toBe(400);
    const switchedToExternal = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/managed-only`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${adminCredential}`, "Content-Type": "application/json" },
      body: JSON.stringify({ sourceType: "external", sourceUrl: "https://example.com/subscription" }),
    }, env);
    expect(switchedToExternal.status).toBe(200);
    const switchedBack = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/managed-only`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${adminCredential}`, "Content-Type": "application/json" },
      body: JSON.stringify({ sourceType: "managed" }),
    }, env);
    expect(switchedBack.status).toBe(200);
    expect(await switchedBack.json()).toMatchObject({ data: { sourceType: "managed", sourceUrl: null } });
    const standaloneYaml = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/managed-only`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${adminCredential}`, "Content-Type": "application/json" },
      body: JSON.stringify({ managedContent: "proxies:\n  - name: example" }),
    }, env);
    expect(standaloneYaml.status).toBe(200);
    const invalidMix = await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/managed-only`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${adminCredential}`, "Content-Type": "application/json" },
      body: JSON.stringify({ sourceType: "external", sourceUrl: "https://example.com/subscription" }),
    }, env);
    expect(invalidMix.status).toBe(422);
    await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/managed-only`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${adminCredential}` },
    }, env);
    await createApp().request(`/${ADMIN_ROUTE}/api/subscriptions/external-only`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${adminCredential}` },
    }, env);
