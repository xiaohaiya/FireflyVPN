(() => {
  ...
  const titles = { dashboard: "概览", subscriptions: "订阅源", accounts: "用户账号", devices: "设备管理", analytics: "统计分析", settings: "运行设置", audit: "审计日志" };
  ...
  const sourceTypeLabel = value => ({ managed: "托管", external: "外部" })[value] || value || "未知";
  ...
  const auditActions = {
    ...
    "admin.access.update": "更新管理端访问参数",
    "admin.login": "管理端登录",
    "admin.logout": "管理端退出",
    "audit.cleanup": "清理审计日志",
  };
  const auditActionLabel = value => auditActions[value] || value || "未知操作";
  const auditDetail = detail => {
    if (!detail || typeof detail !== "object" || Array.isArray(detail)) return detail ?? "-";
    const keyLabels = {
      accessPath: "访问路径",
      tokenChanged: "令牌已修改",
      jwtSecretChanged: "JWT 私钥已修改",
      all: "全部清理",
      from: "开始时间",
      to: "结束时间",
      deleted: "清理条数",
      ...
      enabled: "已启用",
      noticeEnabled: "公告已启用",
      appUpdateEnabled: "应用更新已启用",
      pcAppUpdateEnabled: "PC 端更新已启用",
      bytes: "字节数",
      lines: "行数",
      format: "格式",
    };
    return JSON.stringify(Object.fromEntries(Object.entries(detail).map(([key, value]) => {
      const translated = (key === "from" || key === "to") && typeof value === "string"
        ? dateTime(value)
        ...
        : key === "status"
          ...
          : key === "sourceType"
            ...
            : value;
      return [keyLabels[key] || key, translated];
    })));
  };
  ...
  function trafficChart(daily, today, compact = false) {
    ...
    const totalPoints = rows.map((row, index) => `${x(index)},${y(Number(row.uploadBytes || 0) + Number(row.downloadBytes || 0))}`).join(" ");
    const area = rows.length ? `${margin.left},${margin.top + plotHeight} ${totalPoints} ${x(rows.length - 1)},${margin.top + plotHeight}` : "";
    const labelEvery = Math.max(1, Math.ceil(rows.length / 6));
    const todayX = rows.length ? x(rows.length - 1) : width - margin.right;
    ...
    return `<div class="legend"><span><i class="upload"></i>上传</span><span><i></i>下载</span><span>阴影：上传 + 下载</span><span>悬停数据点可看精确值</span></div><div class="chart-wrap"><svg class="chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="最近三十日上传和下载流量折线图">
      <rect class="today-band" x="${Math.max(margin.left, todayX - 13)}" y="${margin.top}" width="26" height="${plotHeight}"></rect>
      ...
      ${area ? `<polygon class="area-total" points="${area}"></polygon><polyline class="line-upload" points="${points("uploadBytes")}"></polyline><polyline class="line-download" points="${points("downloadBytes")}"></polyline>` : ""}
  ...
  async function loadSubscriptions() {
    ...
    content.querySelectorAll("[data-edit]").forEach(button => button.onclick = async () => {
      ...
      const detail = await runAction(button, {
        working: row.sourceType === "managed" ? "读取正文中…" : "读取中…",
        success: "订阅源已读取",
        failure: "读取订阅源失败",
  ...
  function renderSourceEditor(row) {
    ...
    editor.innerHTML = `<form class="panel form-grid" id="source-form"><h2 class="wide">${row ? "编辑订阅源" : "新增订阅源"}</h2><label class="source-main-field">订阅源标识<input name="id" ${row ? "readonly" : ""} required pattern="[a-z0-9][a-z0-9_-]{0,63}" maxlength="64" value="${escapeHtml(row?.id || "")}"><small>输入的大写字母会自动转成小写</small></label><label class="source-main-field">名称<input name="name" required maxlength="100" value="${escapeHtml(row?.name || "")}"><small>用于控制台和客户端展示</small></label><div class="source-meta-row wide"><label>备注<input name="note" maxlength="200" value="${escapeHtml(row?.note || "")}" placeholder="可选，最多 200 个字符"></label><label>类型<select name="sourceType"><option value="managed" ${row?.sourceType === "managed" ? "selected" : ""}>托管</option><option value="external" ${row?.sourceType === "external" ? "selected" : ""}>外部</option></select></label><label>排序<input name="sortOrder" type="number" value="${row?.sortOrder ?? 0}"></label></div><label>缓存秒数<input name="cacheTtlSeconds" type="number" min="30" max="86400" value="${row?.cacheTtlSeconds || 300}"></label><label>状态<select name="enabled"><option value="true" ${enabled ? "selected" : ""}>启用</option><option value="false" ${!enabled ? "selected" : ""}>停用</option></select></label><label class="wide source-specific" id="source-url-field">外部订阅地址<input name="sourceUrl" type="url" maxlength="2048" value="${escapeHtml(row?.sourceUrl || "")}" placeholder="https://..."></label><label class="wide source-specific" id="managed-content-field">托管订阅正文<textarea class="source-content" name="managedContent" placeholder="粘贴订阅正文">${escapeHtml(row?.managedContent || "")}</textarea></label><div class="form-actions wide"><button class="primary" type="submit">保存</button><button class="secondary" type="button" id="cancel-source">取消</button></div></form>`;
    const form = document.querySelector("#source-form");
    const idInput = form.elements.id;
    const typeInput = form.elements.sourceType;
    const sourceUrlInput = form.elements.sourceUrl;
    const managedContentInput = form.elements.managedContent;
    const sourceUrlField = document.querySelector("#source-url-field");
    const managedContentField = document.querySelector("#managed-content-field");
    const syncSourceType = () => {
      const external = typeInput.value === "external";
      sourceUrlField.hidden = !external;
      managedContentField.hidden = external;
      sourceUrlInput.required = external;
      managedContentInput.required = !external && (!row || row.sourceType !== "managed");
    };
    ...
    form.onsubmit = async event => {
      ...
      if (sourceType === "external") payload.sourceUrl = String(values.get("sourceUrl") || "");
      const managedContent = String(values.get("managedContent") || "");
      if (sourceType === "managed" && managedContent) payload.managedContent = managedContent;
  ...
  async function loadSettings() {
    ...
    content.innerHTML = `<form id="admin-access-form" class="panel form-grid">
      <div class="panel-head wide"><div><h2>管理端访问</h2><p class="panel-subtitle">管理令牌只保存 SHA-256 哈希；JWT 私钥用于签名且均不会回显</p></div><button class="primary" type="submit">保存全部设置</button></div>
      ...
      <label class="admin-token-field">新管理令牌（Admin Token）<input name="newAdminToken" type="password" minlength="8" maxlength="16" autocomplete="new-password" placeholder="留空表示不修改"><small>长度为 8–16 个字符；${adminAccess.adminTokenConfigured ? "当前使用运行时哈希配置" : "当前使用部署环境变量"}</small></label>
      <label class="admin-token-field">确认新管理令牌<input name="confirmAdminToken" type="password" minlength="8" maxlength="16" autocomplete="new-password" placeholder="再次输入；不修改时留空"><small>请与左侧新管理令牌保持一致</small></label>
    ...
    <form id="settings-form" class="panel form-grid">
      <div class="panel-head wide"><div><h2>运行设置</h2><p class="panel-subtitle">修改并保存后，配置将立即生效</p></div><button class="primary" type="submit">保存全部设置</button></div>
    ...
    const renderedRichText = new WeakMap();
    document.querySelectorAll("[data-rich-text-mode]").forEach(button => {
      button.onclick = () => {
        ...
        if (previewing) {
          preview.style.height = `${Math.max(textarea.offsetHeight, 112)}px`;
          if (renderedRichText.get(preview) !== textarea.value) {
            preview.srcdoc = richTextPreviewDocument(textarea.value);
            renderedRichText.set(preview, textarea.value);
          }
        }
    ...
    document.querySelector("#admin-access-form").onsubmit = async event => {
      ...
      if (newAdminToken !== confirmation) {
        notify("保存失败：两次输入的管理令牌不一致", "bad", 7000);
        return;
      }
      ...
      if (!confirm("更新后旧访问路径或旧管理令牌将立即失效，确定继续吗？")) {
        notify("已取消保存管理端访问设置", "busy");
        return;
      }
      const result = await runAction(event.submitter, {
        working: "保存中…",
        success: newAdminToken || newJwtSecret ? "安全参数已更新，正在退出登录…" : "访问路径已更新，正在跳转…",
        failure: "保存管理端访问设置失败",
      }, () => api("/settings/admin-access", {
        method: "PUT",
        body: JSON.stringify({
          accessPath,
      ...
      location.assign(`/${encodeURIComponent(result.accessPath)}/`);
    };
  ...
  async function load(next = page) {
    page = next; title.textContent = titles[page]; document.querySelectorAll("nav button").forEach(button => button.classList.toggle("active", button.dataset.page === page)); setStatus("加载中…", "busy");
